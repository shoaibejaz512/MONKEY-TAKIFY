...
const AdminDashborad = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (taskData) {
                ...
                await firebase.updateAdminTask(taskData.id, updatedData);
                toast.success("Task Updated Successfully ✅");
            } else {
                // ✅ Naya task banane ke liye
                await firebase.adminTask(title, date, description);
            ...
            navigate("/admin-task"); // update ke baad list page par bhej do
        } catch (error) {
    ...
    return (
        <div className='flex relative w-full h-screen justify-center items-center gap-2'>
            ...
            <Sidebar />
            <div className="right-sidebar absolute  top-0 right-0 w-[75%] h-[100vh] bg-[#040247]">
                <div className="header border-b-2 border-white w-full h-[80px] bg-[#020139]">
                    <div className="logo  flex justify-between items-center  w-full h-full">
                        ...
                        <button onClick={ () => {
                            playClickSound();
                            handlelogOut()
                        }} className='bg-violet-600 rounded-sm hvr-wobble-to-top-right  hover:bg-violet-800 duration-300' >LogOut</button>
                    </div>
                </div>
                <div className="form-container w-[100%] h-[100%]">
                    <form onSubmit={handleSubmit}>
                        ...
                        <button onClick={playClickSound} type='submit' className='bg-green-600 hvr-wobble-to-top-right  hover:bg-green-700 duration-300 font-extrabold mx-3'>{taskData ? "Update Task" : "Create Task"}</button>
                    </form>
                </div>
            </div>
        </div>
